...
FootballZombie::FootballZombie(int row, int y, QWidget* parent) {
	this->curPic = new QLabel(parent);
	this->curPic->setAttribute(Qt::WA_TransparentForMouseEvents);
	this->moviePic.resize(2);
	this->moviePic[NORMAL] = FootballZombieWalkPic;
	this->moviePic[ATTACKING] = FootballZombieAttackingPic;
	...
	this->walkSpeed = 1;
	this->attackDistance = 13;
	this->sumAttackTime = 0;
	this->attackIntervals = 600;
	this->attackPower = 30;

}
bool FootballZombie::zombieMoving(bool& isGameOver) {
...
void FootballZombie::zombieReleaseSkill(Plant* (&v)[6][10], vector<pair<int, int>>& diePlant) {
