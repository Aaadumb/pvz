...
IronDoorZombie::IronDoorZombie(int row, int y, QWidget* parent) {
	this->curPic = new QLabel(parent);
	this->curPic->setAttribute(Qt::WA_TransparentForMouseEvents);
	this->moviePic.resize(2);
	this->moviePic[NORMAL] = IronDoorZombieWalkPic;
	this->moviePic[ATTACKING] = IronDoorZombieAttackingPic;
...
bool IronDoorZombie::zombieMoving(bool& isGameOver) {
...
void IronDoorZombie::zombieReleaseSkill(Plant* (&v)[6][10], vector<pair<int, int>>& diePlant) {
